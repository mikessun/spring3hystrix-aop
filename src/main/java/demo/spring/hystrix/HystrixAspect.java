...
public class HystrixAspect {
    ...
    @Around("@annotation(demo.spring.hystrix.HystrixCircuitBreaker)")
    public Object circuitBreakerAround(final ProceedingJoinPoint aJoinPoint) {
        log.info("circuitBreakerAround1");
        String execute = new RemoteServiceCommand(config, aJoinPoint).execute();
        log.info("circuitBreakerAround2, {}",execute);
        return execute;
    }
